#!/usr/bin/env bash
# bin/waktusolat-watcher.sh
# Dual-loop controller for Xmobar parsing reminder.json
...
# --- SUB-JOB 1: Generator & Blinking Engine (Background Loop) ---
(
    while true; do
        if [[ -f "$REMINDER_JSON" && -s "$REMINDER_JSON" ]]; then
            ...
            # Write directly to destination file in-place to retain constant inode
            if [[ -n "$rendered" ]]; then
...
trap "kill $GEN_PID 2>/dev/null; exit 0" EXIT INT TERM
